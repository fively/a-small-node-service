...
export class KnexCoreModule implements OnApplicationShutdown {
  ...
  async onApplicationShutdown(): Promise<any> {
    const connection = this.moduleRef.get<Knex>(getConnectionToken(this.options as KnexModuleOptions) as Type<Knex>);
  ...
  public static createAsyncOptionsProvider(options: KnexModuleAsyncOptions): Provider {
    ...
    const inject = [(options.useClass || options.useExisting) as Type<KnexOptionsFactory>];
